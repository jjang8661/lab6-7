...
func main() {
	...
	router.GET("/query1", func(c *gin.Context) {
		...
		// put your query here
		rows, err := db.Query("SELECT genre, COUNT(Song.title) AS songCount FROM Song JOIN Album ON Song.albumId = Album.albumId GROUP BY Album.genre") // <--- EDIT THIS LINE
		if err != nil {
		...
		for rows.Next() {
			...
			table += "<tr><td>" + genre + "</td><td>"+strconv.Itoa(songCount)+"</td></tr>" // <--- EDIT THIS LINE
		}
